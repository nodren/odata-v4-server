...
export class FilterVisitor implements VisitorMap {
  ...
  protected VisitBoolParenExpression(node: Token, context: any) {
    const inner = this.Visit(node.value, context);
    return (a) => !!inner(a);
  }
  ...
  protected VisitFilter(node: Token, context: any) {
    const predicate = this.Visit(node.value, context);
    return (a) => !!predicate(a);
  }

  protected VisitNotExpression(node: Token, context: any) {
    const expression = this.Visit(node.value, context);
    return (a) => !expression(a);
  }

  protected VisitEqualsExpression(node: Token, context: any) {
    const [left, right] = this.VisitBinaryExpression(node, context);
  ...
  protected VisitParenExpression(node: Token, context: any) {
    const fn = this.Visit(node.value, context);
    return (a) => fn(a);
  }
  ...
  protected getLiteral(node: Token): any {
    switch (node.value) {
      ...
      case 'Edm.Duration':
        const m = node.raw.match(/P([0-9]*D)?T?([0-9]{1,2}H)?([0-9]{1,2}M)?([\.0-9]*S)?/);
        if (m) {
          const d = new Date(0);
  ...
  protected VisitMethodCallExpression(node: Token, context: any) {
    const method = ODataMethodMap[node.value.method];
    const params = (node.value.parameters || []).map((p) => this.Visit(p, context));
    if (!method) {
  ...
  protected VisitIsOfExpression(node: Token, context: any) {
    const type = this.Visit(node.value.typename, context);
    if (!node.value.target) {
      return (a) => a.constructor.name === type(a);
    }
    const target = this.Visit(node.value.target, context);
  ...
  protected VisitArray(node: Token, context: any) {
    const items = (node.value.items || []).map((item) => this.Visit(item, context));
    return (a) => [...items.map((item) => item(a))];
  }

  protected VisitNegateExpression(node: Token, context: any) {
    const exp = this.Visit(node.value, context);
    return (a) => -exp(a);
  }
}
