...
import { doOrderby, doSkip, doTop, processQueries } from './utils/queryOptions';
...
const products2 = require('./model/products').slice();
...
const foobarObj = { id: 1, foo: 'bar' };
const foobarObj2 = { id: 2, foo: 'bar', a: 'a' };
@odata.type(Foobar)
...
export class ImagesController extends ODataController {
  ...
  getData(@odata.key _: number, @odata.context context: ODataHttpContext) {
    const globalReadableImgStrBuffer = new streamBuffers.ReadableStreamBuffer();
  ...
  postData(@odata.key _: number, @odata.body data: Readable) {
    const globalWritableImgStrBuffer = new streamBuffers.WritableStreamBuffer();
...
const globalWritableMediaStrBuffer = new streamBuffers.WritableStreamBuffer();
const globalReadableMediaStrBuffer = new streamBuffers.ReadableStreamBuffer();
@odata.type(Music)
...
export class CategoriesAdvancedGeneratorController extends ODataController {
  ...
  *find(@odata.query query: Token, @odata.filter filter: Token) {
    const options = yield processQueries(query);
...
export class UpsertTestEntityController extends ODataController {
  ...
  put(@odata.body body: any) {
    const up = new UpsertTestEntity(1, 'upsert');
    ...
    return new UpsertTestEntity(randomPort(), body.name);
  }
}
...
serverCache.push(TestServer.create(randomPort()));
...
serverCache.push(ProductServer.create(randomPort()));

@odata.cors
@odata.controller(ProductsController, false)
@odata.controller(CategoriesController, true)
export class CategoryServer extends ODataServer { }
serverCache.push(CategoryServer.create(randomPort()));
