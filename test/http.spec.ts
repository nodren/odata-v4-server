          ...
          try {
            ...
            if (compare.body) {
              if (typeof compare.body == 'object') {
                expect(result).toMatchObject(JSON.parse(JSON.stringify(compare.body)));
              } else {
                expect(result).toEqual(compare.body);
              }
            }
            if (compare.statusCode) {
              expect(response.statusCode).toEqual(compare.statusCode);
            }
...
describe('OData HTTP', () => {
  ...
  describe('accept header', () => {
    ...
    it("should return entityset result with 'application/json;odata.metadata=none' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: 'application/json;odata.metadata=none' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        value: [{
          'a': 1,
          'id': 1
    ...
    it("should return entityset result with 'text/html;odata.metadata=full' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: 'text/html;odata.metadata=full' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        '@odata.context': 'http://localhost:3002/$metadata#EntitySet',
        value: [{
          '@odata.id': 'http://localhost:3002/EntitySet(1)',
    ...
    it("should return entityset result with 'text/html;odata.metadata=minimal' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: 'text/html;odata.metadata=minimal' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        '@odata.context': 'http://localhost:3002/$metadata#EntitySet',
        value: [{
          '@odata.id': 'http://localhost:3002/EntitySet(1)',
    ...
    it("should return entityset result with 'text/html;odata.metadata=none' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: 'text/html;odata.metadata=none' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        value: [{
          'a': 1,
          'id': 1
    ...
    it("should return entityset result with 'xml;odata.metadata=full' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: 'xml;odata.metadata=full' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        '@odata.context': 'http://localhost:3002/$metadata#EntitySet',
        value: [{
          '@odata.id': 'http://localhost:3002/EntitySet(1)',
    ...
    it("should return entityset result with 'xml;odata.metadata=minimal' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: 'xml;odata.metadata=minimal' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        '@odata.context': 'http://localhost:3002/$metadata#EntitySet',
        value: [{
          '@odata.id': 'http://localhost:3002/EntitySet(1)',
    ...
    it("should return entityset result with 'xml;odata.metadata=none' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: 'xml;odata.metadata=none' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        value: [{
          'a': 1,
          'id': 1
    ...
    it("should return entityset result with '*/*;odata.metadata=full' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: '*/*;odata.metadata=full' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        '@odata.context': 'http://localhost:3002/$metadata#EntitySet',
        value: [{
          '@odata.id': 'http://localhost:3002/EntitySet(1)',
    ...
    it("should return entityset result with '*/*;odata.metadata=minimal' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: '*/*;odata.metadata=minimal' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        '@odata.context': 'http://localhost:3002/$metadata#EntitySet',
        value: [{
          '@odata.id': 'http://localhost:3002/EntitySet(1)',
    ...
    it("should return entityset result with '*/*;odata.metadata=none' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: '*/*;odata.metadata=none' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        value: [{
          'a': 1,
          'id': 1
    ...
    it("should return entityset result with '*/*;odata.metadata=minimal;charset=utf-16' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: '*/*;odata.metadata=minimal;charset=utf-16' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        '@odata.context': 'http://localhost:3002/$metadata#EntitySet',
        value: [{
          '@odata.id': 'http://localhost:3002/EntitySet(1)',
    ...
    it("should return entityset result with '*/*;odata.metadata=none;charset=utf-16' header", () => request.get(`http://localhost:3002/EntitySet`, { headers: { accept: '*/*;odata.metadata=none;charset=utf-16' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        value: [{
          'a': 1,
          'id': 1
    ...
    it("should create new foo element with 'application/json;odata.metadata=full' header", () => request.post(`http://localhost:3002/EntitySet`, { headers: { accept: 'application/json;odata.metadata=full' }, json: { id: 999, foo: '999' } }, (err, response, result) => {
      expect(response.statusCode).toEqual(201);
    }).then((_) => request.get(`http://localhost:3002/EntitySet(999)`, { headers: { accept: 'application/json;odata.metadata=none' } }, (err, response, result) => {
      expect(JSON.parse(result)).toEqual({
        id: 999,
        foo: '999'
      });
    ...
    it.skip("should return expanded Meta with 'application/json;odata.metadata=full' header using $select", () => request.get(`http://localhost:3003/Meta(MongoId='578f2b8c12eaebabec4af242',Id=1,p9=9,p10=10)?$select=Id&$expand=Meta.Meta/MediaList`, { headers: { accept: 'application/json;odata.metadata=full' } }, (err, response, result) => {
    ...
    it.skip("should return expanded Meta with 'application/json;odata.metadata=full' header", () => request.get(`http://localhost:3003/Meta(MongoId='578f2b8c12eaebabec4af242',Id=1,p9=9,p10=10)?$expand=Meta.Meta/MediaList`, { headers: { accept: 'application/json;odata.metadata=full' } }, (err, response, result) => {
      expect(JSON.parse(result)).toMatchObject({
    ...
    it("should return expanded Meta with 'application/json;odata.metadata=none' header", () => request.get(`http://localhost:3003/Meta(MongoId='578f2b8c12eaebabec4af242',Id=1,p9=9,p10=10)?$expand=Meta.Meta/MediaList`, { headers: { accept: 'application/json;odata.metadata=none' } }, (err, response, result) => {
      expect(JSON.parse(result)).toMatchObject({
        'Id': 1,
        'MediaList': [
          {
    ...
    it('should return error if the media type is unsupported', () => request.get(`http://localhost:3003/EntitySet`, { headers: { accept: 'text/plain;odata.metadata=none' } }, (err, response, result) => {
      expect(JSON.parse(result).error.message).toEqual('Unsupported media type.');
    }).catch((ex) => {
      if (ex) { expect(JSON.parse(ex.error).error.message).toEqual('Unsupported media type.'); }
    }));

    it('should return error if odata-maxversion less then 4.0', () => request.get(`http://localhost:3002/EntitySet`, { headers: { 'odata-maxversion': '3.0', accept: '*/*;odata.metadata=full' } }, (err, response, result) => {
      expect(JSON.parse(result).error.message).toEqual('Only OData version 4.0 supported');
    }).catch((ex) => {
      if (ex) { expect(JSON.parse(ex.error).error.message).toEqual('Only OData version 4.0 supported'); }
    }));

    it('set status code 403 in controller', () => request.get(`http://localhost:3002/HeaderTestEntity`, (err, response, result) => {
      expect(response.statusCode).toEqual(403);
    }).catch((ex) => {
      if (ex) { return expect(ex.statusCode).toEqual(403); }
    }));

    it('set status code 500 in controller', () => request.get(`http://localhost:3002/HeaderTestEntity(2)`, (err, response, result) => {
      expect(response.statusCode).toEqual(500);
    }).catch((ex) => {
      if (ex) { return expect(ex.statusCode).toEqual(500); }
    }));

    it('set status code 403 in FunctionImport', () => request.get(`http://localhost:3002/SetStatusCode()`, (err, response, result) => {
      expect(response.statusCode).toEqual(403);
    }).catch((ex) => {
      if (ex) { return expect(ex.statusCode).toEqual(403); }
    }));

    it('set status code 500 in ActionImport', () => request.post(`http://localhost:3002/SetStatusCode2`, (err, response, result) => {
      expect(response.statusCode).toEqual(500);
    }).catch((ex) => {
      if (ex) { return expect(ex.statusCode).toEqual(500); }
    }));
  ...
  it("should update foobar's foo property ", () => request.put(`http://localhost:3002/EntitySet(1)/foo`, { json: { foo: 'PUT' } }, (err, response, result) => {
    expect(response.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/EntitySet(1)`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
  ...
  it("should delete foobar's foo property ", () => request.delete(`http://localhost:3002/EntitySet(1)/foo`, (err, response, result) => {
    expect(response.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/EntitySet(1)`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
      '@odata.context': 'http://localhost:3002/$metadata#EntitySet/$entity',
  ...
  it("should delta update foobar's foo property ", () => request.patch(`http://localhost:3002/EntitySet(1)/foo`, { json: { foo: 'bar' } }, (err, response, result) => {
    expect(response.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/EntitySet(1)`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
      '@odata.context': 'http://localhost:3002/$metadata#EntitySet/$entity',
  ...
  it("should delete foobar's 'a' property with PATCH handler", () => request.delete(`http://localhost:3002/EntitySet(2)/a`, (err, response, result) => {
    expect(response.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/EntitySet(2)`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
      '@odata.context': 'http://localhost:3002/$metadata#EntitySet/$entity',
  ...
  it('should create product reference on category', () => request.post(`http://localhost:3002/Categories('578f2baa12eaebabec4af28e')/Products('578f2b8c12eaebabec4af242')/$ref`, (err, response, result) => {
    expect(response.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/Products('578f2b8c12eaebabec4af242')/Category`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
  ...
  it('should delete product reference on category', () => request.delete(`http://localhost:3002/Categories('578f2baa12eaebabec4af28e')/Products('578f2b8c12eaebabec4af242')/$ref`, (err, req, result) => {
    expect(req.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/Products('578f2b8c12eaebabec4af242')/Category`, (err, req, result) => {
    if (err) { return expect(err.name).toEqual('ResourceNotFoundError'); }
  }).catch((ex) => {
    if (ex) { return expect(ex.statusCode).toEqual(404); }
  })));

  it('should update product reference on category', () => request.put(`http://localhost:3002/Categories('578f2baa12eaebabec4af28d')/Products('578f2b8c12eaebabec4af242')/$ref`, (err, response, result) => {
    expect(response.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/Products('578f2b8c12eaebabec4af242')/Category`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
      '@odata.context': 'http://localhost:3002/$metadata#Categories/$entity',
  ...
  it('should delete product reference on category by ref id', () => request.delete(`http://localhost:3002/Categories('578f2baa12eaebabec4af28b')/Products/$ref?$id=http://localhost:3002/Products('578f2b8c12eaebabec4af284')`, (err, req, result) => {
    expect(req.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/Products('578f2b8c12eaebabec4af284')/Category`, (err, req, result) => {
    if (err) { return expect(err.name).toEqual('ResourceNotFoundError'); }
  }).catch((ex) => {
    if (ex) { return expect(ex.statusCode).toEqual(404); }
  })));

  it('should delta update product reference on category', () => request.patch(`http://localhost:3002/Categories('578f2baa12eaebabec4af28b')/Products('578f2b8c12eaebabec4af284')/$ref`, (err, req, result) => {
    expect(req.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/Products('578f2b8c12eaebabec4af284')/Category`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
      '@odata.context': 'http://localhost:3002/$metadata#Categories/$entity',
  ...
  it('should create category reference on product', () => request.post(`http://localhost:3002/Products('578f2b8c12eaebabec4af286')/Category/$ref`, { json: { '@odata.id': "http://localhost:3002/Categories(categoryId='578f2baa12eaebabec4af28c')" } }, (err, req, result) => {
    expect(req.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/Products('578f2b8c12eaebabec4af286')/Category`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
      '@odata.context': 'http://localhost:3002/$metadata#Categories/$entity',
  ...
  it('should delete category reference on product', () => request.delete(`http://localhost:3002/Products('578f2b8c12eaebabec4af286')/Category/$ref`, { json: { '@odata.id': "http://localhost:3002/Categories('578f2baa12eaebabec4af28c')" } }, (err, req, result) => {
    expect(req.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/Products('578f2b8c12eaebabec4af286')/Category`, (err) => {
    if (err) { return expect(err.name).toEqual('ResourceNotFoundError'); }
  }).catch((ex) => {
    if (ex) { return expect(ex.statusCode).toEqual(404); }
  })));

  it('should update category reference on product', () => request.put(`http://localhost:3002/Products('578f2b8c12eaebabec4af286')/Category/$ref`, { json: { '@odata.id': "http://localhost:3002/Categories(categoryId='578f2baa12eaebabec4af289')" } }, (err, req, result) => {
    expect(req.statusCode).toEqual(204);
  }).then((_) => request.get(`http://localhost:3002/Products('578f2b8c12eaebabec4af286')/Category`, (err, response, result) => {
    expect(JSON.parse(result)).toEqual({
      '@odata.context': 'http://localhost:3002/$metadata#Categories/$entity',
  ...
  describe('Media entity', () => {
    it('media entity POST', (done) => {
      const readableStrBuffer = new streamBuffers.ReadableStreamBuffer();
      const req = request.post(`http://localhost:3002/MusicControllerEntitySet(1)/$value`);
      readableStrBuffer.pipe(req);
      readableStrBuffer.put('tmp.png');
      readableStrBuffer.stop();
      req.on('error', (err) => {
