...
import { createODataServer, Edm, odata, ODataController, ODataProcessor, ODataQuery, ODataServer, ODataStream } from '../src';
...
export class MetaController extends ODataController {
  ...
  findAll(@odata.context __: any, @odata.result ___: any, @odata.stream ____: ODataProcessor) {
    return [
      { MongoId: new ObjectID('5968aad95eb7eb3a94a264f6'), b0: 'basemeta', '@odata.type': BaseMeta },
...
export class SimpleEntityController extends ODataController {
  ...
  findAll(@odata.key key: string) {
    const simple = new SimpleEntity();
    simple.MongoId = new ObjectID('randomPort()aad95eb7eb6b94a354g7');
    return [simple];
  }
...
serverCache.push(SchemaJsonServer.create('/schemaJsonTest', randomPort()));

class DefineEntitiesServer extends ODataServer { }
DefineEntitiesServer.$metadata(defineEntities);
serverCache.push(DefineEntitiesServer.create('/defineEntitiesTest', randomPort()));
...
serverCache.push(TypeDefServer.create(randomPort()));
...
serverCache.push(EnumServer.create(randomPort()));
...
serverCache.push(MetaTestServer.create(randomPort()));
serverCache.push(MetaTestServer.create('/test', randomPort()));
serverCache.push(createODataServer(MetaTestServer, '/test', randomPort()));
...
if (typeof describe == 'function') {
  describe('Metadata test', () => {
    it('should return metadata xml', () => {
      expect(beautify(MetaTestServer.$metadata().document())).toEqual(
    ...
    it('should return SchemaJsonServer metadata xml', () => {
      expect(beautify(SchemaJsonServer.$metadata().document())).toEqual(
    ...
    it('should return DefineEntitiesServer metadata xml', () => {
      expect(beautify(DefineEntitiesServer.$metadata().document())).toEqual(
    ...
    it('should return TypeDefServer metadata xml', () => {
      expect(beautify(TypeDefServer.$metadata().document())).toEqual(
    ...
    it('should return EnumServer metadata xml', () => {
      expect(beautify(EnumServer.$metadata().document())).toEqual(
    ...
    it('should return ActionFunctionServer metadata xml', () => {
      expect(beautify(ActionFunctionServer.$metadata().document())).toEqual(
