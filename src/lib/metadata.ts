    ...
    const parameters = Edm.getParameters(elementType, prop).map((p) => {
      ...
      if (typeof param.type != 'string') {
        ...
        if (Edm.isTypeDefinition(param.type)) {
          param.type = resolveTypeDefinition(containerType, resolvedContainerType, namespace, Edm.getType, ():string => Edm.getTypeName(containerType, resolvedContainerType, server.container));
