...
exports.createMetadataJSON = void 0;
...
function createMetadataJSON(server) {
        ...
        const parameters = Edm.getParameters(elementType, prop).map((p) => {
            ...
            if (typeof param.type != 'string') {
                ...
                if (Edm.isTypeDefinition(param.type)) {
                    param.type = resolveTypeDefinition(containerType, resolvedContainerType, namespace, Edm.getType, () => Edm.getTypeName(containerType, resolvedContainerType, server.container));
                }
                    ...
                    if (typeof param.type == 'function') {
                        ...
                        const resolvedType = resolveType(param.type, elementType, prop, undefined, resolvedContainerType.indexOf('.') > 0 ? resolvedContainerType : `${param.type.namespace || namespace}.${resolvedContainerType}`);
                        if (resolvedType && !resolvedType.schema[definitionContainer].filter((et) => et.name == resolvedType.definition.name)[0]) {
                            resolvedType.schema[definitionContainer].push(resolvedType.definition);
                        }
                    }
    ...
    const resolveType = (elementType, parent, prop, baseType, paramTypeName) => {
        ...
        const properties = Edm.getProperties(elementType.prototype);
        ...
        const operations = getAllOperations(elementType);
    ...
    propNames.forEach((i) => {
        if (i != '$metadata' && server.prototype[i]) {
            if (server.prototype[i].prototype instanceof controller_1.ODataController) {
                ...
                const operations = getAllOperations(ctrl);
                Object.keys(operations).forEach((operation) => {
